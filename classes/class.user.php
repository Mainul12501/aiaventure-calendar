<?php
   ...
   * @copyright 2017-2023
   * @version $Id: class.user.php, v1.2 Feb 2023 transinova $
   */
...
if($numassigned>0){$assgnum='<span class="badge text-bg-secondary">'.$numassigned.'</span>';}else{$assgnum='-';}						  
						  $company = ($usr->company)?$usr->company:'-';
						  $disp .='<tr>
						  <td>'.$usr->fname.' '.$ustatus.'<br /><a href="javascript:void(0);" class="opentimodal">'.$this->userLevelName($usr->userlevel).'</a></td>
...
$disp = '
        <form class="form" id="form_useredit">
          <div class="row">
            ...
            <div class="col-sm-4">
              <div class="form-group">
                <label style="width:100%" for="userlevel">'.Lang::$say->_UR_UROLE.' <a href="javascript:void(0);" class="float-end opentimodal"><span class="glyphicon glyphicon-question-sign"></span> '.Lang::$say->_HELP.'</a>  </label>
...
foreach ($pptassignments as $ppty) {
	...
	if($typeName){$typeList .= '<div class="col-sm-12 col-md-6 col-lg-4"><h3 class="badge text-bg-secondary intd">'.$typeName.'</h3></div>';}
}
unset($ppty);
...
if($usr->active!='y'){$ustatus='<span class="glyphicon glyphicon-ban-circle recstatus" aria-hidden="true"></span>';}else{$ustatus='';}						  
						  $company = ($usr->company)?$usr->company:'-';
						  $disp .='<tr>
						  <td width="200"><strong>'.$usr->fname.'</strong> '.$ustatus.'<br /><span class="small">'.'<a href="javascript:void(0);"  class="opentimodal">'.$this->userLevelName($usr->userlevel).'</a>'.'<br />'.$usr->username.'<br />'.$usr->email.'<br />'.$usr->company.'</span></td>
