<?php
   ...
   * @copyright 2017-2023
   * @version $Id: class.calendar.php, v1.2 Feb 2023 transinova $
   */
...
<div class="card-header"><h3 class="card-title"><span class="moncalnum float-end"> '. str_pad(intval($mn) , 2 , "0" , STR_PAD_LEFT) . '</span>' . $this->arrMonths[$mn][$this->monthNameLength] .'  <span class="d-inline-block d-sm-none"> '. $cdate['year'] . '</span></h3></div>
  <div class="card-body">
...
if($this->isCanCreateBlock()){$addbtn='<button class="btn btn-default float-end bkad" data-cont="bookingadd" href="javascript:void(0);" data-ppttype="' . $this->ppttype . '" data-y="'. $this->prevYear['year'] . '"  data-vmode="yearlist">'.$iconadd.' '.Lang::$say->_BKG_CREATE.'</button>';}else{$addbtn='';}
	$calnav = '
		  <div class="row calnav gx-3">
		    <div class="calnav-controlwrap col-sm-4">
    <select id="categories" name="category" class="form-control">
        <option disabled selected >Select a category</option>
        <option value="Economy Sedan" >Economy Sedan</option>
        <option value="Economy MPV">Economy MPV</option>
        <option value="Business Sedan">Business Sedan</option>
        <option value="Business MPV">Business MPV</option>
        <option value="Luxury sedan">Luxury sedan</option>
        <option value="Luxury MPV">Luxury MPV</option>
    </select>
</div>
		  <div class="calnav-controlwrap col-sm-4">
		  <select class="form-control changetype" name="changetype" data-auth="'.genRequestKey('calNavigateYear'.$cdate['year']).'" data-year="' . $cdate['year'] . '" data-vmode="yearlist" data-live-search="true">
		  '.Vault::get("Vhrental")->propertyTypeSelectBox($ppttype).'
		  </select>
		  </div><div class="calnav-controlwrap col-sm-2">
...
if($counter>0){$calnav .= '<a target="_blank" class="float-end exportmodebutton noprint" href="printmode.php?do=blocks2csv&p=' . $eppid . '&y=' . $cdate['year'] . '"><span class="glyphicon glyphicon-save"></span></a>';}
$calnav .= '</div>
		  <div class="calnav-controlwrap col-sm-2">'.$addbtn.'</div>
		  </div>
		  ';
...
private function renderYearCalendar($year=false,$ppttype=false)
	  {
		...
		for ($mn = 1; $mn < 13; $mn++) {
		  ...
		  <div class="wtb-sm">';
		  $rend .= '<div class="wtbcaption"><span class="moncalnum float-end"> '. str_pad(intval($mn) , 2 , "0" , STR_PAD_LEFT) . '</span><div class="wtbrow">';
...
if($islocked){$iconadd='<span class="glyphicon glyphicon-lock" aria-hidden="true"></span>';}else{$iconadd='<span class="glyphicon glyphicon-plus" aria-hidden="true"></span>';}
if($this->isCanCreateBlock()){$addbtn='<button class="btn btn-default float-end bkad" data-cont="bookingadd" href="javascript:void(0);" data-ppttype="' . $ppttype . '" data-y="'. $cdate['year'] . '">'.$iconadd.' '.Lang::$say->_BKG_CREATE.'</button>';}else{$addbtn='';}
    ...
    <select id="categories" name="category" class="form-control">
        ...
        <option value="Luxury MPV">Luxury MPV</option>
		  ...
		  <div class="calnav-controlwrap col-sm-4">
		  <select class="form-control changetype" id="changetype1" name="changetype" data-auth="'.genRequestKey('calNavigateYear'.$cdate['year']).'" data-year="' . $cdate['year'] . '" data-live-search="true">
		  '.Vault::get("Vhrental")->propertyTypeSelectBox($ppttype).'
		  </select>
		  </div><div class="calnav-controlwrap col-sm-2">
		  ...
		  <div class="calnav-controlwrap col-sm-2">'.$addbtn.'<a class="float-end printbutton" href="javascript:window.print();"><span class="glyphicon glyphicon-print"></span></a></div>
		  </div>	  
		  ';
...
if($months==1 || $months==2 || $months==3 || $months==4){
		  $mn=$month;			
          $day = 1;//$day = date("d");
		  $ldim = $this->calcDays($mn, $day);
		  $cdate = getdate(mktime(0, 0, 0, $mn, $day, $year));
	...
	$calnav = '
		  <div class="row calnav gx-3">
		  <div class="calnav-controlwrap col-sm-12">';//$months
	if(!$psview||$psview=='null'){$calnav .= '
		  <select class="form-control changetypemonth" name="changetype" data-auth="'.genRequestKey('calNavigateMonth'.$cdate['mon'].$cdate['year']).'" data-year="' . $cdate['year'] . '" data-month="' . $cdate['mon'] . '" data-months="' . $months . '" data-live-search="true">
		  '.Vault::get("Vhrental")->propertyTypeSelectBox($ppttype).'
		  </select>
		  ';}else{$calnav .= '<h4>'.$ppttypename.'</h4>';}
		  $calnav .= '</div><div class="calnav-controlwrap col-sm-12">
...
if(!$months || $months==12){
    ...
    <select id="categories" name="category" class="form-control">
        ...
        <option value="Luxury MPV">Luxury MPV</option>
		  ...
		  <div class="calnav-controlwrap col-md-4">';
	if(!$psview||$psview=='null'){$calnav .= '
		  <select class="form-control changetypeyear" name="changetype" data-auth="'.genRequestKey('calNavigateYearView'.$cdate['year']).'" data-year="' . $cdate['year'] . '" data-view="12" data-live-search="true">
		  '.Vault::get("Vhrental")->propertyTypeSelectBox($ppttype).'
		  </select>
		  ';}else{$calnav .= '<h4>'.$ppttypename.'</h4>';}
		  $calnav .='</div><div class="calnav-controlwrap col-md-4">
...
//2022
$datee=explode('-',$date);
if(checkdate($datee[1], $datee[2], $datee[0])){
          ...
          <div class="row">
            <div class="col-sm-4">
              <div class="form-group">
                <label for="checkin">'.Lang::$say->_BKG_TRASTR.'</label>
            ...
            <div class="col-sm-4">
              <div class="form-group">
                <label for="checkout">'.Lang::$say->_BKG_TRAEND.'</label>
...
<div class="row">
          <div class="col-sm-6">
              <div class="form-group">
                <label for="guestname">'.Lang::$say->_BKG_MEMNAME.'</label>
                <input type="text" class="form-control" name="guestname" value="'.$row->guestname.'">
              </div>
          </div>
           <div class="col-sm-3">
           <div class="form-group">
                <label for="guestadult">'.Lang::$say->_BKG_MEMID.'</label>
...
<div class="row">
           ...
           <div class="col-sm-4">
               <div class="form-group">
                <label for="guestcountry">'.Lang::$say->_BKG_PURPOSE.'</label>
                <select name="guestcountry" id="" class="form-control">
                    <option value="Tour" '.$row->guestCountry == "Tour" ? "selected" : " ".'>Tour</option>
                    <option value="Family trip" '.$row->guestCountry == "Tour" ? "selected" : " ".'>Family trip</option>
                    <option value="Business trip" '.$row->guestCountry == "Tour" ? "selected" : " ".'>Business trip</option>
                    <option value="Others" '.$row->guestCountry == "Tour" ? "selected" : " ".'>Others</option>
                </select>
              </div>             
            </div>
</div>
...
$btn='<a class="float-end printmodebutton" target="_blank" href="printmode.php?do=block&i='.Vault::get("niceIds")->makenice($row->detbookingid).'"><span class="glyphicon glyphicon-print"></span></a>';$btnprint='';
if($row->denote){
...
else{$col='col-sm-12';$cols='';$clas='';$btn='';$btnprint='<h5>'.Lang::$say->_BKG_BLKDETAILS.'<a class="float-end printbutton" href="javascript:window.print();"><span class="glyphicon glyphicon-print"></span></a></h5>';
...
$disp='';
if($this->isCanCreateBlock()){
$disp ='<div class="modal fade" id="modaldlg_bookingadd" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">	
       <h1 class="modal-title fs-5"><span class="glyphicon glyphicon-plus"></span> '.Lang::$say->_BKG_CREATE.'</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="'.Lang::$say->_CLOSE.'"></button>
		</div>
...
<div class="modal fade" id="modaldlg_bookingedit" tabindex="-1" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
       <h1 class="modal-title fs-5"><span class="glyphicon glyphicon-pencil"></span> '.Lang::$say->_BKG_EDIT.'</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="'.Lang::$say->_CLOSE.'"></button>		
		</div>
...
$disp .='
<div class="modal fade" id="modaldlg_bookinginfo" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
       <h1 class="modal-title fs-5"><span class="glyphicon glyphicon-info-sign"></span> '.Lang::$say->_BKG_BLKDETAILS.'</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="'.Lang::$say->_CLOSE.'"></button>
		</div>
...
if (strpos($keyword, '@') !== false) {
    ...
    $mtq=" AGAINST ('" . self::$db->escape($keyword) . "*' IN BOOLEAN MODE) ";
		  ...
		  if($row){
			  ...
			  $rend .= '
<div class="card panel-default">
<div class="card-header"><h3 class="card-title"><span class="moncalnum float-end"> '. str_pad(intval($brow->smonth) , 2 , "0" , STR_PAD_LEFT) . '</span>' . $this->arrMonths[$brow->smonth][$this->monthNameLength] .'  <span class=""> '. $brow->syear . '</span></h3></div>
  <div class="card-body">
    <div class="rowspan yearlist"><table class="restable table table-sm"><tbody>';				  
$checkin = explode("-",$brow->checkin);
$checkout = explode("-",$brow->checkout);
