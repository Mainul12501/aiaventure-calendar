<?php
   ...
   * @copyright 2017-2023
   * @version $Id: class.vhrental.php, v1.2 Feb 2023 transinova $
   */
...
if($row->hideinpub){$hideinpub='<span class="glyphicon glyphicon-eye-close recstatus" aria-hidden="true"></span>';}else{$hideinpub='';}			  
			  ...
			  <div class="row">
			  <div class="col-sm-5">
			    <span class="dd-handle"><span class="glyphicon glyphicon-menu-hamburger" aria-hidden="true"></span></span>'.$row->name.$hidden.$locked.$hideinpub.'</div>
			    <div class="col-md-3">
...
<div class="col-sm-6">
     <div class="form-group">
         <label for="categories">'.Lang::$say->_PPT_SECCATEGORY .'</label>
             <select id="categories" name="category" class="form-control">
                 <option value="Economy Sedan" '.($row->category == "Economy Sedan" ? "selected" : " ") .'>Economy Sedan</option>
                 <option value="Economy MPV" '.($row->category == "Economy MPV" ? "selected" : " ") .' >Economy MPV</option>
                 <option value="Business Sedan" '.($row->category == "Business Sedan" ? "selected" : " ") .' >Business Sedan</option>
                 <option value="Business MPV" '.($row->category == "Business MPV" ? "selected" : " ") .' >Business MPV</option>
                 <option value="Luxury sedan" '.($row->category == "Luxury sedan" ? "selected" : " ") .' >Luxury sedan</option>
                 <option value="Luxury MPV" '.($row->category == "Luxury MPV" ? "selected" : " ") .' >Luxury MPV</option>
             </select>
     </div>
 </div>
<div class="col-sm-6">
...
<div class="row">
<div class="col-sm-9"><label class="hidden-xs label-mb">&nbsp;</label>
<div class="form-group">
...
<div class="col-sm-3"><label class="hidden-xs">&nbsp;</label> 
  ...
  <button id="save_ppttypeedit" data-after="none" type="button" class="btn btn-primary btn-block"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> '.Lang::$say->_SAVE.'</button></div>
</div>    
</div>
</div>  
</form>
...
<div class="col-sm-7">
  <div class="form-group">
    <label for="title'.Lang::$lang.'">'.Lang::$say->_PPT_BLSTTITLE.' '.Lang::$say->_FOR.': '.$row->status.' <span class="badge text-bg-secondary">'.Vault::get("Lang")->currlang.'</label>
    <input type="text" class="form-control" name="title'.Lang::$lang.'" placeholder="'.Lang::$say->_PPT_BLSTTITLE.'" value="'.$row->{'title'.Lang::$lang}.'">
  </div>
</div>
<div class="col-sm-1"><label class="label-mb">'.Lang::$say->_PPT_BLSTCOLOR.'</label>
<div class="form-group">
<input id="color_hex" type="text" name="colorhex" value="'.$row->colorhex.'" />
</div>
</div>
<div class="col-sm-2"><label class="hidden-xs label-mb">&nbsp;</label>
<div class="form-group">
    <label class="checkbox-inline">
...
if(!Vault::get("Users")->logged_in){
		  ...
		  while ($row = self::$db->fetch($query)) {
			  ...
			  if($row->id==$selectid){$selected=' selected="selected"';}else{$selected='';}
			  $disp .='<option'.$selected.' data-tokens="'.strtolower($row->name.$locked).'"  value="'.$row->id.'">'.$row->name.$locked.'</option>';
		  }
...
<label for="changetype_calfeed">'.Lang::$say->_PPT_TYPE.'</label>
        <select class="form-control custom-select changetypes" name="changetype_calfeed" id="changetype_calfeed" data-auth="'. genRequestKey('linkFeedCal'.$userid).'" data-live-search="true">
          <option value="-">-</option>
...
<label for="changetype_calexp">'.Lang::$say->_PPT_TYPE.'</label>
        <select class="form-control changetype custom-select" name="changetype_calexp" id="changetype_calexp" data-auth="'.genRequestKey($userid.'linkExportCal').'" data-live-search="true">
          <option value="-">-</option>
          '.Vault::get("Vhrental")->propertyTypeSelectBox().'
        </select>
</div>		
...
<label for="changetype_publiclinks">'.Lang::$say->_PPT_TYPE.'</label>
        <select class="form-control custom-select changetypes" name="changetype_publiclinks" id="changetype_publiclinks" data-auth="'. genRequestKey('publicLinks'.$userid).'" data-live-search="true">
          <option value="-">-</option>';
...
$top = '<div class="row"><div class="col-md-12"><label class="bolder label-mb">'.Lang::$say->_UR_ASSIGNPPTY.'</label></div></div><div class="row" style="padding-bottom:10px;">';
