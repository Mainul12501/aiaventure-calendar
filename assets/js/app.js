...
$(document).on('click', "[id^=showmodaldlg_]", function(event) {
 ...
 event.stopPropagation();
	...
	var suffix = funcid[1];
	var bkTinoModal = new bootstrap.Modal(document.getElementById('modaldlg_'+suffix), {keyboard: false,backdrop:'static'});
		$('#msg_'+suffix).html('');
		$('#form_'+suffix).show();
            ...
            if (json.status == "success") {
				...
				if(suffix=='langadd' || suffix=='langedit' || suffix=='langdel'){
					$('body').fadeOut(1000, function () { location.reload(true); });
						...
						if($('.changetype').length){$('.changetype').selectpicker();}
						}else{
				if(json.content){$('.listpanel_'+suffix).html(json.content);
								 if($('.changetype').length){$('.changetype').selectpicker();}
								}
...
$(document).on("click", "a.bkiavae", function(event) {	
		...
		+"<a class='bkinf btn btn-primary btn-sm' href='#' data-cont='bookinginfo' data-id='"+dataid+"' data-option='infoBoking'><span class='glyphicon glyphicon-info-sign'></span></a> "
...
$(document).on("click", "a.bkiavai", function(event) {	
		...
		if(chektype=='a'){chektype='';}
		$("#popbkiavanae").html(datadate+"<br />"+chektype+" "+avail+"<div class='wpop_actionbuttons'>"
		+"<a class='bkinf btn btn-primary btn-sm' href='#' data-cont='bookinginfo' data-id='"+dataid+"' data-option='infoBoking'><span class='glyphicon glyphicon-info-sign'></span></a></div> "
		);
...
$(document).on('click', 'a.itemedit', function(event) {	
		...
		event.stopPropagation();
                ...
                var listcont = $(this).data('listcont');
				var edItemModal = new bootstrap.Modal(document.getElementById('modaldlg_'+listcont), {keyboard: false,backdrop:'static'});
                        $.ajax({
                            type: 'post',
                            url: relpath+'controller/controller.php',
                            dataType: 'json',
            ...
            if (json.status == "success") {
                ...
                //$('.msg_'+listcont).html(json.message);
				if(json.content){
				$('#panel_'+listcont).html(json.content);
				edItemModal.show();
...
$(document).on('click', ".bkad", function(event) {
 ...
 event.stopPropagation();
				...
				var bkAddModal = new bootstrap.Modal(document.getElementById('modaldlg_'+suffix), {keyboard: false,backdrop:'static'});
            ...
            if (json.status == "success") {
				...
				if(json.content){
				$('#panel_'+suffix).html(json.content);	
				bkAddModal.show();
				$('#modaldlg_'+suffix).draggable({handle: ".modal-header"});
				}	
...
$(document).on('click', "a.bked", function(event) {
 ...
 event.stopPropagation();
                ...
                var suffix = $(this).data('cont');
				var keyword = $(this).data('keyword');
				var bkEdModal = new bootstrap.Modal(document.getElementById('modaldlg_'+suffix), {keyboard: false,backdrop:'static'});
                        $.ajax({
                            type: 'post',
                            url: relpath+'controller/controller.php',
                            dataType: 'json',
...
$(document).on('click', "a.bkinf", function(event) {
 ...
 event.stopPropagation();
				...
				var vmode = $(this).data('vmode');
                var suffix = $(this).data('cont');
				var bkInfModal = new bootstrap.Modal(document.getElementById('modaldlg_'+suffix), {keyboard: false,backdrop:'static'});
                        $.ajax({
                            type: 'post',
                            url: relpath+'controller/controller.php',
                            dataType: 'json',
...
//--------------------------------------------------
 if($('.changetype').length){$('.changetype').selectpicker();}
 if($('.changetypes').length){$('.changetypes').selectpicker();}
  


$(document).on('click', "a.opentimodal", function(event) {
 event.preventDefault();
 event.stopPropagation(); 
 var modal = document.getElementById("timodal-userrole");  
 modal.style.display = "block";    
}); 
$(document).on('click', "span.timodal-close", function(event) {
 event.preventDefault();
 event.stopPropagation(); 
 var modal = document.getElementById("timodal-userrole");  
...
var timodal = document.getElementById("timodal-userrole");

 window.onclick = function(event) {
  if (event.target == timodal) {
